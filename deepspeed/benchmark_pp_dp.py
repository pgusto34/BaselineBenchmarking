...
def load_deepspeed_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    Args:
        config_path: Path to DeepSpeed config JSON file.
                    If None, uses default 'ds_config_pp_dp.json' in script directory.
    ...
    if config_path is None:
        # Default to ds_config_pp_dp.json in the same directory as this script
        script_dir = Path(__file__).parent
        config_path = script_dir / "ds_config_pp_dp.json"
    ...
    if not config_path.exists():
        raise FileNotFoundError(
            ...
            "Please create a ds_config_pp_dp.json file or specify a valid path."
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to DeepSpeed config JSON file (default: ds_config_pp_dp.json)",
    )
